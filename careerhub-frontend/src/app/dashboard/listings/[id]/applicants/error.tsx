...
export default function ApplicantsErrorBoundary({
  error,
  reset,
}: {
    ...
    return (
      <ErrorShell heading="Session Expired">
        <p className="mt-2 text-sm text-[var(--muted-text)]">
          Your session has expired. Please sign in again to view applicants.
        </p>
  ...
  if (error instanceof ApiError && error.isForbidden) {
    return (
      <ErrorShell heading="Not Your Listing">
        <p className="mt-2 text-sm text-[var(--muted-text)]">
          You can only view applicants for listings your own company posted.
        </p>
        <Link href="/dashboard/listings" className={secondaryLinkCn}>
  ...
  if (error instanceof ApiError && error.code === "NOT_FOUND") {
    return (
      <ErrorShell heading="Listing Not Found">
        <p className="mt-2 text-sm text-[var(--muted-text)]">
