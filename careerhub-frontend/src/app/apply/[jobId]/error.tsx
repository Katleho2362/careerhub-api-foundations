...
export default function ApplyErrorBoundary({
  error,
  reset,
}: {
  ...
  if (error instanceof ApiError && error.isForbidden) {
    return (
      <ErrorShell heading="Candidate Account Required">
        <p className="mt-2 text-sm text-[var(--muted-text)]">
          Only candidate accounts can apply for jobs. Employers cannot submit
          applications.
        </p>
  ...
  if (error instanceof ApiError && error.code === "NOT_FOUND") {
    return (
      <ErrorShell heading="Job Not Found">
        <p className="mt-2 text-sm text-[var(--muted-text)]">
          This listing may have been removed or closed.
        </p>
        <Link href="/jobs" className={secondaryLinkCn}>
          Browse open roles
        </Link>
