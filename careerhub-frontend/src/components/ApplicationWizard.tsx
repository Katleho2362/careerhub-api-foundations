...
// Maps a server-side field name to the step that owns it, so a 422 can
// navigate the user back to the step containing the invalid field rather
// than leaving them stranded on the confirm step. Built once, outside the
// component, since it's derived from the static STEP_FIELDS map above.
function stepForField(fieldName: string): number {
  for (const [step, fields] of Object.entries(STEP_FIELDS)) {
    if ((fields as string[]).includes(fieldName)) {
      return Number(step);
    }
  }
  // Unknown field name (e.g. a server-only field with no matching UI
  // input) — default to step 1 rather than leaving the user on step 3
  // with no visible way to see the error.
  return 1;
}
...
export function ApplicationWizard({
  ...
  const mutation = useMutation({
    ...
    onError: (err: Error) => {
      // 422 validation: map each server field error back onto the form,
      // navigate to the step that owns the first invalid field, and show
      // a review toast instead of a generic failure message.
      if (err instanceof ApiError && err.isValidation && err.fields) {
        const fieldNames = Object.keys(err.fields);

        fieldNames.forEach((fieldName) => {
          const messages = err.fields![fieldName];
          setError(fieldName as keyof WizardInput, {
            type: "server",
            message: messages[0],
          });
        });
        ...
        if (fieldNames.length > 0) {
          const targetStep = stepForField(fieldNames[0]);
          setStep(targetStep);
        }
      ...
      // Any non-validation error falls through to the existing generic toast.
  ...
  async function onSubmit(data: WizardOutput) {
    try {
      ...
      // already handled by mutation.onError (toast / field errors);
      // swallow here so react-hook-form's handleSubmit doesn't see an
      // unhandled rejection.
    }
  }
