...
function formatSalary(
  ...
  const fmt = new Intl.NumberFormat("en-ZA", { maximumFractionDigits: 0 });
  if (minNum !== null && maxNum !== null) {
    return `R${fmt.format(minNum)} – R${fmt.format(maxNum)} / mo`;
  }
  const single = minNum ?? maxNum ?? 0;
  return `R${fmt.format(single)} / mo`;
}

function relativeDate(iso: string | undefined): string {
  // postedAt is optional in the generated JobResponse schema — the
  // backend's [Required] attributes don't cover it, so a listing could
  // theoretically come back without one. "Recently" is a safe fallback
  // that never invents a date the API didn't actually send.
...
export function JobCard({ job, isSelected, onSelect }: JobCardProps) {
  const isActive = isJobActive(job);
  // id is optional in the generated JobResponse schema. In practice the
  // API always returns one for a real listing, but the type no longer
  // guarantees it, so onSelect needs a non-undefined string to satisfy
  // its signature. Falling back to "" rather than loosening onSelect's
  // type keeps every other caller of onSelect simple.
  const jobId = job.id ?? "";
